...
World.prototype.update = function(dt) {
	...
	for(var i = 0; i < this.agents.length; i++) {
		...
		if(agent.typeId == "termite"){
			newAgents = newAgents.concat(agent.anglesToUpdate);
			agent.anglesToUpdate = [];
		}
	}
...
World.prototype.processCollisions = function() {
	...
	for(var i = 0; i < this.agents.length - 1; i++) {
		...
		for(var j = i+1; j < this.agents.length; j++) {
			...
			if(this.agentsCollidingOLD(agent1, agent2)) {
			...
			if(agent1.typeId == "wood_heap" && agent2.typeId == "termite"){
				this.processPerceptionOLD(agent1, agent2);
			}
			else{
...
World.prototype.processPerceptionOLD = function(agent1, agent2) {
...
World.prototype.agentsCollidingOLD = function(agent1, agent2) {
	if(agent1.boundingRadius > 0 && agent2.boundingRadius > 0) {
		return this.processCircleCircleCollisionOLD(agent1, agent2);
	} else if(agent1.boundingRadius > 0 && agent2.boundingWidth > 0) {
		return this.processCircleRectangleCollisionOLD(agent1, agent2);
	} else if(agent2.boundingRadius > 0 && agent1.boundingWidth > 0) {
		return this.processCircleRectangleCollisionOLD(agent2, agent1);
	} else if(agent2.boundingWidth > 0 && agent1.boundingWidth > 0) {
		return this.processRectangleRectangleCollisionOLD(agent1, agent2);
	}
	return false;
};

World.prototype.processCircleCircleCollisionOLD = function(agent1, agent2) {
	var deltaVect = new Vect(agent1.x - agent2.x, agent1.y - agent2.y);
...
World.prototype.processCircleRectangleCollisionOLD = function(agent1, agent2) {
...
World.prototype.processRectangleRectangleCollisionOLD = function(agent1, agent2) {
